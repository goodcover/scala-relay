...
import com.goodcover.relay.build.GraphQLConverter.Options
import sbt.*
import sbt.util.CacheImplicits.*
import com.goodcover.relay.build.{ GraphQLConverter as SharedQLConverter, GraphQLSchema }
import sbt.util.{ CacheStore, CacheStoreFactory }
import sjsonnew.*

/**
 * Converts GraphQL definitions to Scala.js facades to be used with the sources
...
object GraphQLConverter {
  ...
  def convertFiles(files: Set[File], schema: GraphQLSchema, options: Options, logger: Logger) =

    SharedQLConverter.convertFiles(files, schema, options, SbtBuildLogger(logger))
  ...
  /**
   * If the version has changed this will delete all previous conversions and
   * re-convert everything. Otherwise it will remove old conversions for removed
   * resources and convert anything that is new or was modified.
   *
   * @return
   *   a tuple where the first element are the new conversion and the second are
   *   the conversions from the previous analysis that have not been modified
   */
