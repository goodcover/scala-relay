...
object GraphQLExtractor {
      ...
      Tracked.diffInputs(stores.sources, FileInfo.lastModified)(sources) { sourcesReport =>
        ...
        Tracked.diffOutputs(stores.outputs, FileInfo.lastModified)(outputs) { outputsReport =>
          ...
          if (unexpectedChanges.nonEmpty) {
            ...
            val needsExtraction    = unexpectedChanges.foldLeft(Map.empty[File, File]) { case (acc, output) =>
  ...
  /**
   * If the version has changed this will delete all previous extracts and
   * re-extract everything. Otherwise it will remove old extracts for removed
   * sources and extract from anything that is new or was modified.
   *
   * @return
   *   a tuple where the first element are the files that were extracted and the
   *   second are all the extracts from the previous analysis that have not been
   *   modified
   */
  private def extractModified(
    ...
    if (versionUnchanged && optionsUnchanged) {
      ...
      val modifiedAndTransitiveOutputs = modifiedOutputs.flatMap { case entry @ (_, output) =>
        entry +: unmodifiedSources.getOrElse(output, Vector.empty).map(_ -> output)
      }
