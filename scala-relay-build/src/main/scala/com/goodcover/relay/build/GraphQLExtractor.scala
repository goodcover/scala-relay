 ...
 * macros within Scala sources. This is a build-tool agnostic version that can
 * be used by both SBT and Mill.
 */
object GraphQLExtractor {
  ...
  /**
   * Extract GraphQL definitions from source files without caching. This is a
...
//  }
   ...
   * callers responsibility to ensure that existing extracts are deleted
   * beforehand if required.
   */
  def extractFiles(files: Map[File, File], dialect: Dialect, logger: BuildLogger): Map[File, File] =
    files.filter { case (file, output) =>
      Try(extractFile(file, output, dialect, logger)).fold(
        { t =>
          logger.warn(
