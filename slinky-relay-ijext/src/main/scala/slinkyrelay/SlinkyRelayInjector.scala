...
class SlinkyRelayInjector extends SyntheticMembersInjector {
  ...
  override def needsCompanionObject(source: ScTypeDefinition): Boolean = {
    source.findAnnotationNoAliases("slinkyrelay.annotations.reactRelay") != null
  }
  ...
  def createExternalBody(cls: ScTypeDefinition): Seq[(String, InjectType)] = {
    val applyMethods = cls.extendsBlock.members
      .collect {
      ...
      .map {
        case propsCls: ScClass if propsCls.isCase =>
          ...
          if (paramList.params.forall(_.isDefaultParam)) {
            ...
            Seq(s"def apply${paramList.getText}: slinky.core.BuildingComponent[$element, $ref] = ???" -> Function)
          }
  ...
  def createFunctionalComponentBody(cls: ScTypeDefinition): Seq[(String, InjectType)] = {
    val applyMethods = cls.extendsBlock.members
      ...
      .flatMap { elm =>
        elm match {
          case alias: ScTypeAliasDefinition =>
            Some(Seq(s"def apply(props: ${cls.name}.Props): ${cls.name}.component.Result = ???" -> Function))
          case propsCls: ScClass if propsCls.isCase =>
            Some {
              val paramList        = propsCls.constructor.get.parameterList
              val caseClassparamss = paramList.params
              ...
              if (childrenParam.isDefined) {
                if (paramssWithoutChildren.isEmpty) {
                  Seq(s"def apply(${childrenParam.get.getText}): ${cls.name}.component.Result = ???" -> Function)
                } else {
                  Seq(
  ...
  def isSlinky(tpe: ScTypeDefinition): Boolean = {
    tpe.findAnnotationNoAliases("slinkyrelay.annotations.reactRelay") != null
  }
